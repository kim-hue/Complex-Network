...
% Triangle formation for Directed cycle vs. undirected K3
%This function will call formODE for solving ode45 purposes
%f1 can be kept constant and f2 modified to observe the difference in the 
%result by changing feedback parameters 
%-------------------------------------------------------------------------% 
clear;clc;close all;
...
L = kron(Lg,eye(2*n));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%For Triangle%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %triangle formation model
r = 1.0;
...
y3 = pos(:,6);
% Plot results
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%For Triangle%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% figure(2);
h1 = plot(x1(1),y1(1),'x',x2(1),y2(1),'x',x3(1),y3(1),'x');hold on;
h2 = plot(x1,y1,x2,y2,x3,y3);hold on;
h3 = plot(x1(end),y1(end),'o',x2(end),y2(end),'o',x3(end),y3(end),'o');hold on
